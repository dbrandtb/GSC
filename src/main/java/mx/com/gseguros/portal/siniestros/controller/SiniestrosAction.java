    	...
    	if(causaSiniestro !=null){
        	if(!causaSiniestro.equalsIgnoreCase(CausaSiniestro.ACCIDENTE.getCodigo())){
        		if(circuloHospAsegurado == null){
        			...
        			if(circuloHospAsegurado.equalsIgnoreCase("PLUS 100") || circuloHospAsegurado.equalsIgnoreCase("B")) {valor1="1";}
                	if(circuloHospAsegurado.equalsIgnoreCase("PLUS 500") || circuloHospAsegurado.equalsIgnoreCase("A")) {valor1="2";}
                	if(circuloHospAsegurado.equalsIgnoreCase("PLUS 1000") || circuloHospAsegurado.equalsIgnoreCase("PLUS"))
                	{
                		valor1="3";
                	}
                	else{
    		...
    		if(TipoPago.DIRECTO.getCodigo().equals(tramite.get("OTVALOR02")))//if(esPagoDirecto)
    		{
    			...
    			for(int i = 0; i < facturasAux.size(); i++)
    			{
    				...
    				for( int j= 0; j < siniestros.size();j++){
        				...
        				if(informacionGral.size()>0){
        					aseguradoObj.put("CAUSASINIESTRO", informacionGral.get(0).get("CDCAUSA"));
        					//Verificamos la causa del siniestro Si es Accidente, verificamos si tiene contratada la poliza de 
        				...
        				if(calculosPenalizaciones.equalsIgnoreCase("1")){//if(calculosPenalizaciones.equalsIgnoreCase("1")){
        		   			//4.1.- Verificamos si existe exclusión de penalización
        		   			HashMap<String, Object> paramExclusion = new HashMap<String, Object>();
            		   		paramExclusion.put("pv_cdunieco_i",cdunieco);
            		   		paramExclusion.put("pv_estado_i",estado);
        				...
        				String sDeducibleSiniestroIte     = copagoDeducibleSiniestroIte.get("DEDUCIBLE").replace(",","");
        				logger.debug("VALOR DE sDeducibleSiniestroIte -->"+sDeducibleSiniestroIte);
        				String sCopagoSiniestroIte        = copagoDeducibleSiniestroIte.get("COPAGO").replace(",", "");
        				String tipoCopagoSiniestroIte     = copagoDeducibleSiniestroIte.get("TIPOCOPAGO");
        				...
        				if(cdramo.toString().equalsIgnoreCase("7")){
        					//GMMI
        				...
        				if(tipoFormatoCalculo.equalsIgnoreCase("1"))
    					{
    						...
    						if(!causadelSiniestro.equalsIgnoreCase("2")){
    							...
    							//accidente
    							logger.debug("VALOR DEL RAMO ACCIDENTE --->"+cdramo);
    							if(cdramo.equalsIgnoreCase("7")){
    								logger.debug("VALOR DEL RAMO ACCIDENTE ---> 7 existeCobertura"+existeCobertura);
