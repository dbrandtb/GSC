...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
            <item-list xsi:type="java:mx.com.aon.portal.model.DesglosePolizasVO" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
            ...
            <xsl:if test="@CDTIPCON">
                <xsl:element name="cd-tip-con">
                    <xsl:value-of select="@CDTIPCON"/>
                </xsl:element>
            </xsl:if>
            <xsl:if test="@DSTIPCON">
                <xsl:element name="ds-tip-con">
