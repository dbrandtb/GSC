...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="rows">
		<xsl:for-each select="row">
			<item-list
				xsi:type="java:mx.com.aon.portal.model.RequisitoCancelacionVO"
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                <xsl:if test="@CDRAZON">
                ...
                <xsl:if test="@CDADVERT">
                    <xsl:element name="cd-advert">
                        <xsl:value-of select="@CDADVERT" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@DSADVERT">
                    <xsl:element name="ds-advert">
                        <xsl:value-of select="@DSADVERT" />
                    </xsl:element>
                </xsl:if>            
			</item-list>
		</xsl:for-each>
