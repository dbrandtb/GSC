...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:java="http://xml.apache.org/xslt/java" exclude-result-prefixes="java">
    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
            <item-list xsi:type="java:mx.com.aon.portal.model.DescuentoProductoVO" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
		        <xsl:if test="@CDDSCTO">
		        ...
		        <xsl:if test="@DSNOMBREP">
		            <xsl:element name="ds-nombre">
		                <xsl:value-of select="@DSNOMBREP" />
		            </xsl:element>
		        </xsl:if>
                ...
                <xsl:if test="@FEREGIST">
		            <xsl:element name="fe-regist">
		                <xsl:value-of select="@FEREGIST" />
		            </xsl:element>
		        </xsl:if>
		        <xsl:if test="@FEINACTI">
		            <xsl:element name="fe-inacti">
		                <xsl:value-of select="@FEINACTI" />
		            </xsl:element>
		        </xsl:if>
            </item-list>
        </xsl:for-each>
