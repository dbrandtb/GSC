...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="rows">
		<xsl:for-each select="row">
			<item-list
				xsi:type="java:mx.com.aon.portal.model.CarritoComprasProductosVO"
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
				...
				<xsl:if test="@DSRAZSOC">
					<xsl:element name="ds-raz-soc">
						<xsl:value-of select="@DSRAZSOC" />
					</xsl:element>
				</xsl:if>
				<xsl:if test="@FEINISUS">
					<xsl:element name="fe-ini-sus">
						<xsl:value-of select="@FEINISUS" />
					</xsl:element>
				</xsl:if>
				<xsl:if test="@FEFINSUS">
					<xsl:element name="fe-fin-sus">
						<xsl:value-of select="@FEFINSUS" />
					</xsl:element>
				</xsl:if>
				<xsl:if test="@NMTOTAL">
					<xsl:element name="nm-totalp">
