...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
            <item-list xsi:type="java:mx.com.aon.catbo.model.TipoGuionVO" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                ...
                <xsl:if test="@DSMODULO">
                    <xsl:element name="descrip-corta">
                        <xsl:value-of select="@DSMODULO" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@NMMODULO">
                    <xsl:element name="descrip-larga">
                        <xsl:value-of select="@NMMODULO" />
                    </xsl:element>
                </xsl:if>
            </item-list>
        </xsl:for-each>
