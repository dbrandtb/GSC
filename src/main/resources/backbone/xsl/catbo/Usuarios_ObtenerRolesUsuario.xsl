...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="rows">
		<xsl:for-each select="row">
			<item-list
				xsi:type="java:mx.com.aon.catbo.model.RolesVO"
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
				<xsl:if test="@IDGRUPO">
					<xsl:element name="id-rol">
						<xsl:value-of select="@IDGRUPO" />
					</xsl:element>
				</xsl:if>
				<xsl:if test="@DESCRIPCIONROL">
					<xsl:element name="nombre-rol">
						<xsl:value-of select="@DESCRIPCIONROL" />
					</xsl:element>
				</xsl:if>
			</item-list>
		</xsl:for-each>
