    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
        	<xsl:if test="@TIPOOBJETO = 1">
        		<item-lista xsi:type="java:com.biosnet.ice.ext.elements.form.SimpleCombo" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                    ...
                    <xsl:if test="@OTTABVAL">
                        <xsl:element name="store">
                            store<xsl:value-of select="@OTTABVAL"/>
                        </xsl:element>
                    </xsl:if> 
	            </item-lista>
        	</xsl:if>
        	<xsl:if test="@TIPOOBJETO = 2">
	            <item-lista xsi:type="java:com.biosnet.ice.ext.elements.form.TextFieldControl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
