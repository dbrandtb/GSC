                    ...
                    <children xsi:type="java:com.biosnet.ice.ext.elements.model.RamaMaster" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">                        
                        ...
                        <xsl:for-each-group select="current-group()" group-by="@DSPROPIEDAD">
                                    <xsl:if test="@DSPROPIEDAD='NombreTecnico'">                                        
                                        <xsl:element name="id"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    
                                    <xsl:if test="@DSPROPIEDAD='Etiqueta'">
                                        <xsl:element name="text"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    
                                    <xsl:if test="@DSPROPIEDAD='Agrupador'">
                                        <xsl:element name="agrupador"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>                                    
                                    
                                    <xsl:if test="@DSPROPIEDAD='OrdenAgrupacion'">
                                        <xsl:element name="orden-agrupacion"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                        ...
                        <xsl:if test="@OTVALOR = '2'">                            
                            <ext-control xsi:type="java:com.biosnet.ice.ext.elements.form.TextFieldControl">
                                ...
                                <xsl:for-each-group select="current-group()" group-by="@DSPROPIEDAD">
                                    <xsl:if test="@DSPROPIEDAD='NombreTecnico'">
                                        <xsl:element name="name"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                        <xsl:element name="id"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    ...
                                    <xsl:if test="@DSPROPIEDAD='LongitudMinima'">
                                        <xsl:if test="@OTVALOR>'0'">
                                            ...
                                            <xsl:element name="min-length-text">El mínimo de caracteres es <xsl:value-of select="@OTVALOR" /></xsl:element>
                                        </xsl:if>
                                    </xsl:if>
                                    ...
                                    <xsl:if test="@DSPROPIEDAD='LongitudMaxima'">
                                        <xsl:if test="@OTVALOR>'0'">
                                            ...
                                            <xsl:element name="max-length-text">El máximo de caracteres es <xsl:value-of select="@OTVALOR" /></xsl:element>
                                        </xsl:if>
                                    </xsl:if>
                                    
                                    <xsl:if test="@DSPROPIEDAD='UsarcomoFiltro'">
                                        <xsl:element name="usarcomo-filtro"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    
                                    <xsl:if test="@DSPROPIEDAD='Etiqueta'">
                                        <xsl:element name="field-label"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    
                                    <xsl:if test="@DSPROPIEDAD='Obligatorio'">
                                    	<xsl:if test="@SWCOMOBL='S'">
					                        <xsl:element name="allow-blank">false</xsl:element>
					                    </xsl:if>
					                    <xsl:if test="@SWCOMOBL='N'">
                            ...
                            <ext-control xsi:type="java:com.biosnet.ice.ext.elements.form.ComboMaster">
                                ...
                                <xsl:for-each-group select="current-group()" group-by="@DSPROPIEDAD">                                    
                                    <xsl:if test="@DSPROPIEDAD='NombreTecnico'">
                                        <xsl:element name="name"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                        <xsl:element name="id"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                        <xsl:element name="hidden-name">h<xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    <xsl:if test="@DSPROPIEDAD='ListaValores'">
                                        <xsl:element name="backup-table"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
                                    <xsl:if test="@DSPROPIEDAD='UsarcomoFiltro'">
                                        <xsl:element name="usarcomo-filtro"><xsl:value-of select="@OTVALOR" /></xsl:element>
                                    </xsl:if>
