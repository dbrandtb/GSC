...
var nmPoliza = new Ext.form.TextField({
  ...
  var fgBFP = new Ext.grid.CheckColumn({
    ...
    var eleccionColumn = new Ext.grid.CheckColumn({
      id:'eleccionColumnId',
      header: getLabelFromMap('eleccionColumnId',helpMap,'Sel'),
...
var grid = new Ext.grid.EditorGridPanel ({
   		...
   		plugins: [fgCancelar,eleccionColumn],//fgbfp
   		...
   		buttons:[
          		...
          		tooltip:getToolTipFromMap('chkListCtaBttAdd', helpMap,'Guardar'),
           		handler:function(){
           		                   //recs=Ext.getCmp('gridId').getSelectionModel().selectAll();
           						var params  = "cdRamo="+_cdRamo;
								...
								conn.request ({
								   		...
								   		callback: function (options, success, response) {    
								       			...
								       			if(validacion==true){                				
									               		guardarDatosPolizas(storeGrillaPolizasCancelar);
									   			}else{
...
function guardarDatosPolizas (elStore) {
  var _seleccionados=0; 
  var _params = "";
  for (var i=0; i<elStore.getCount(); i++) {
  	if ((elStore.getAt(i).data.seleccionar)== true){
	   _params +=   "csoGrillaList[" + i + "].cdUnieco=" + elStore.getAt(i).data.cdUnieco + "&" +
	      "&csoGrillaList[" + i + "].cdRamo=" + elStore.getAt(i).data.cdRamo + "&" +
	      "&csoGrillaList[" + i + "].estado=" + elStore.getAt(i).data.estado + "&" +
	      "&csoGrillaList[" + i + "].nmPoliza=" + elStore.getAt(i).data.nmPoliza + "&" +
	      "&csoGrillaList[" + i + "].nmsuplem=" + elStore.getAt(i).data.nmsuplem + "&" +
	      "&csoGrillaList[" + i + "].cdagrupa=" + elStore.getAt(i).data.cdagrupa + "&" +
	      "&csoGrillaList[" + i + "].nmrecibo=" + elStore.getAt(i).data.nmrecibo + "&" +
	      "&csoGrillaList[" + i + "].cdcancel=" + elStore.getAt(i).data.cdcancel + "&" +
	      "&csoGrillaList[" + i + "].status=" + elStore.getAt(i).data.status + "&" +
	      "&csoGrillaList[" + i + "].feCancel=" + elStore.getAt(i).data.feCancel + "&" +
	      "&csoGrillaList[" + i + "].swcancela=" + ((elStore.getAt(i).data.swcancela == true)?'S':'N')  + "&";
	      	++_seleccionados;
  	}
  }
