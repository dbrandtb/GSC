      function muestraDetalle(record){//Variables temporales que se usarán como parametros para la url flujocotizacion/comprarCotizaciones.action 
        ...
        /*tmpCodigoAseguradora = record.get('codigoAseguradora');
        tmpCodigoProducto = record.get('codigoProducto');
        tmpEstado = record.get('estado');
        tmpNmpoliza = record.get('nmpoliza');
        tmpNmsituac = record.get('nmsituac');*/

		//nombreAseguradora.setValue(record.get('dsUniEcoId'));
        //nombrePlan.setValue(record.get('dsplan'));
        //nombrePrima.setValue(record.get('prima'));
