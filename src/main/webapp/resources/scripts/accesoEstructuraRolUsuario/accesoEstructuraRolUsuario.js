...
function agregar(_record){
	...
	var ventana = new Ext.Window ({
	   	title: '<span style="color:black;font-size:12px;">'+getLabelFromMap('83', helpMap,'Agregar Acceso')+'</span>',
	    width: 500,
	    modal: true,
	    items: formWindow,
	    buttonAlign: "center",
...
Ext.onReady(function(){
	...
	var cm = new Ext.grid.ColumnModel([
        ...
        {
        header: getLabelFromMap('cmUsuarioDs',helpMap,'Nombre de persona'),
        tooltip: getToolTipFromMap('cmUsuarioDs',helpMap,'Nombre de persona'),
        dataIndex: 'dsUsuario',
        width: 125,
        sortable: true
        }
	...
	var grid = new Ext.grid.GridPanel({
            ...
            buttons:[
                  ...
                  tooltip: getToolTipFromMap('gridBtn3',helpMap,'Exporta los datos del listado en un determinado formato'),
                   handler:function(){
                        var url = _ACTION_EXPORTAR_ACCESO_ESTRUCTURA_ROL_USUARIO + '?dsNivel=' + formPanel.findById('txtNivelId').getValue() + '&dsRol=' + formPanel.findById('txtRolId').getValue() + '&dsUsuario=' + formPanel.findById('txtUsuarioId').getValue() + '&dsEstado=' + formPanel.findById('txtEstadoId').getValue();
	...
	function borrar(_record) {
		if(_record){		   
			Ext.MessageBox.confirm(getLabelFromMap('400000', helpMap,'Aviso'), getLabelFromMap('400031', helpMap,'Se elimin&oacute; el registro seleccionado'),function(btn){
		        if (btn == "yes"){
