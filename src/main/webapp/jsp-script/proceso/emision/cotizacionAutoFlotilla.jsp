...
//                                                     }
    ...
    try{
    	if(_p30_smap1.turistas=='S'){
	    	...
	    	_fieldByName('aux.otvalor18',_fieldById("_p30_fieldsetTatripol"),true).setMargin('0 0 0 15');
	    	_fieldById("fsPanel1").insert(Ext.ComponentQuery.query("#fsPanel1>[xtype]").length-2,_fieldByName('aux.otvalor18',_fieldById("_p30_fieldsetTatripol"),true));
	    	
	    	_fieldByLabel('AGENTE',_fieldById("fsPanel1")).on({
	    		select:function(me){
	    			
	    			var extraparams={
	   	    			 catalogo:'RAMO_5_NEGOCIO_X_CDTIPSIT_AGENTE'
	   	    			,'params.cdtipsit':'TL'
	   	    			,'params.producto':'I'
	   	    			,'params.cdsisrol': _GLOBAL_CDSISROL
	   	    			,'params.cdagente':me.getValue() 
	   	    	}
	   	    	_fieldByLabel('NEGOCIO',_fieldById("fsPanel1")).store.proxy.extraParams=extraparams;
	   	    	_fieldByLabel('NEGOCIO',_fieldById("fsPanel1")).store.load();
	    		}
	    	})
                            ...
                            &&_fieldById('_p30_idInputCorreos').getValue() != 'Correo(s) separados por ;')
                    {
                        ...
                        Ext.Ajax.request(
                        {
                            ...
                            ,params :
                            {
                                ...
                                'Se ha realizado la cotización solicitada en el trámite, ' + _p30_smap1.ntramite + ' la cual se anexa para su revisión y/o aprobación.\n \n' +
										...
										,buttons:[{
													...
													,handler : 
														function (me){
															...
															//Se agrega para confirmar cuando sea diferente de Autorizacion
															if(Ext.isEmpty(_p30_flujo)
											                    ||Ext.isEmpty(_p30_flujo.aux)
											                    ||_p30_flujo.aux.indexOf('onComprar')==-1){
											                    	//
											                    	json2.smap1.confirmar = 'si';
											                    }
															storeTvalosit.each(function(record) {
			                                                ...
			                                                Ext.Ajax.request( {
														           ...
														           ,jsonData : json2
															       ,success  : function(response){
															       		var json3 = Ext.decode(response.responseText);debug('***json3',json3);
															       		if(Ext.isEmpty(_p30_flujo)
														                    ||Ext.isEmpty(_p30_flujo.aux)
														                    ||_p30_flujo.aux.indexOf('onComprar')==-1){ //si no hay auxiliar en flujo, o el auxiliar no contiene la palabra onComprar
														                    	...
														                    	debug('_p30_flujoAux.endosoAutorizar: ',_p30_flujoAux.endosoAutorizar);
															                    var ck = 'Turnando tr\u00e1mite';
															                    try{
															                        var status = _p30_flujoAux.endosoAutorizar.split('_')[1];
															                        debug('status para turnar onComprar:',status,'.');
															                        _mask(ck);
															                        Ext.Ajax.request( {
															                            url      : _GLOBAL_COMP_URL_TURNAR
														                    ...
														                    sacaEndoso(_p30_smap1.CDUNIECO
																						,_p30_smap1.CDRAMO
																						,_p30_smap1.ESTADO
																						,_p30_smap1.NMPOLIZA
														                               ,json3.omap1.pv_nmsuplem_o
														                               ,json3.omap1.pv_nsuplogi_o);
														                    }
															       }
														            ,failure  : function(){
														                errorComunicacion();
