...
public class ConfigurarEncuestaTest extends AbstractTestCases {
	protected ConfigurarEncuestaManager configurarEncuestaManager;
	...
	/*
		 public void testObtenerEncuesta() throws Exception {
				try {
					...
					PagedList pagedList =  (PagedList)configurarEncuestaManager.obtenerAsignacionEncuesta("", "", "", "", "", "", 0, 10);
					//pv_dsunieco_i, pv_dsramo_i, pv_dselemento_i, pv_dsproceso_, pv_dscampan_i, pv_dsmodulo_i, pv_dsencuesta_i, start, limit
					//assertNotNull(pagedList);
		            List lista = pagedList.getItemsRangeList();
		            for (int i=0; i<lista.size(); i++){
