...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	...
	/**
	* contiene el mensaje devuelto desde la base de datos
	*
	*/
	,actionMessage: ''

	/**
	* contiene el mensaje de error devuelto desde la base de datos
	*
	*/
	,actionError: ''

	/**
	* @uploadCallback {function} funci�n donde se devuelve el control luego del upload
	*/
	,clientCallback: null
	
	/**
	* @actionResponse {Object} devuelve la respuesta completa desde el action ejecutado
	*/
	,actionResponse: null
	// methods
	// {{{
	...
	,processSuccess:function(options, response, o) {
		...
		if (response && response.responseText) {
			var actionMessage = Ext.util.JSON.decode(response.responseText).actionMessages[0];
	...
	,processFailure:function(options, response, error) {
		...
		this.actionResponse = response;
		if (response && response.responseText) {
			var actionError = Ext.util.JSON.decode(response.responseText).actionErrors[0];
			this.actionError = actionError;
			Ext.Msg.alert('Error', this.actionError);
		}
	...
	,uploadCallback:function(options, success, response) {
		...
		this.clientCallback(success, this.actionResponse);
		this.fireFinishEvents(options);
