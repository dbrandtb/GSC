...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
	...
	/**
	* @maxFileCount {int} cantidad m·xima de archivos a subir
	*/
	,maxFileCount: 0
    
	/**
     * 
     * @hideCloseButton para configurar si la cruz del costado superior derecho del control aparece o no  
     */
	,hideCloseButton: true
	
	/**
	* @uploadCallback {function} funciÛn donde el componente retorna el control cuando termine el upload
	*/
	,uploadCallback: null
	
	/**
	* @hideUploadButton {boolean} Indica si se debe mostrar el botÛn de Cargar Archivo
	*/
	...
	,initComponent:function() {
		...
		var deleteLabel ='Eliminar';
		
		// todo: either to cancel buttons in body or implement it
		if('body' !== this.buttonsAt) {
			if (this.hideCloseButton)
				this[this.buttonsAt] = [addCfg, upCfg, '->'];
			else this[this.buttonsAt] = [addCfg, upCfg, '->',deleteLabel];
		}
		// }}}
		// {{{
		// create store
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
	...
	,getQtip:function(values) {
		...
		switch(values.state) {
			...
			case 'done':
				...
				if (this.uploader.actionMessage && this.uploader.actionMessage != '') {
					Ext.Msg.alert('Aviso', this.uploader.actionMessage);
				}
				qtip += '<br>' + this.clickRemoveText;
			break;
			...
			case 'failed':
				...
				if (this.uploader.actionError && this.uploader.actionError != '') {
					Ext.Msg.alert('Aviso', this.uploader.actionError);
				} else {
					Ext.Msg.alert('Aviso', qtip);
				}
				qtip += '<br>' + this.clickRemoveText;
			break;
	...
	,onAddFile:function(bb) {
		if (this.maxFileCount > 0) {
			if (this.store && this.store.getCount() >= this.maxFileCount) return;
		}
		...
		Ext.getCmp('addCfgId').setDisabled(true);
		if (this.removeAllBtn) this.removeAllBtn.enable();
	...
	,onRemoveFile:function(record) {
		...
		Ext.getCmp('addCfgId').setDisabled(false);
		this.uploadBtn.setDisabled(!count);
		if (this.removeAllBtn) this.removeAllBtn.setDisabled(!count);
	...
	,onUpload:function() {
		...
		this.uploader.clientCallback = this.uploadCallback;
	...
	,updateButtons:function() {
		if(true === this.uploading) {
			...
			if (this.removeAllBtn) this.removeAllBtn.setIconClass(this.stopIconCls);
			if (this.removeAllBtn) this.removeAllBtn.getEl().child(this.removeAllBtn.buttonSelector).dom[this.removeAllBtn.tooltipType] = this.stopAllText;
		}
		else {
			this.addBtn.enable();
			this.uploadBtn.enable();
			if (this.removeAllBtn) this.removeAllBtn.setIconClass(this.removeAllIconCls);
			if (this.removeAllBtn) this.removeAllBtn.getEl().child(this.removeAllBtn.buttonSelector).dom[this.removeAllBtn.tooltipType] = this.removeAllText;
		}
