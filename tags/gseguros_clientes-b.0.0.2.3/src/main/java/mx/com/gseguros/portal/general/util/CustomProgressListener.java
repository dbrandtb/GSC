...
public class CustomProgressListener implements ProgressListener {
	...
	private boolean contentLengthKnown = false;
        ...
        private org.apache.log4j.Logger log=org.apache.log4j.Logger.getLogger(CustomProgressListener.class);
	...
	public void update(long bytesRead, long contentLength, int items) {
		...
		//long nowNum100Ks = bytesRead / 100000;
                long nowNum100Ks = bytesRead / 10;
		// Only run this code once every .01K
		if (nowNum100Ks > num100Ks) {
			num100Ks = nowNum100Ks;
			if (contentLengthKnown) {
				percentDone = (int) Math.round(100.00 * bytesRead / contentLength);
                                if(contentLengthKnown&&bytesRead==contentLength)
                                {
                                    log.debug("Upload finish "+System.currentTimeMillis());
                                    estado=TERMINADO;
                                }
			}
