...
Ext.ux.PagingToolbar = Ext.extend(Ext.Toolbar, {
	 ...
	 */
	variations: [10, 25, 50, 100, 250, 500],
	
	// ------ end add for rows per paging ---------
	...
	updateStore: function(){
	    // TODO - I can not get total count at first time load
	    var total = 1000;
		...
		for (var i=0, len=v.length; i<len; i++){
			if(total>v[i]) {this.addToStore(v[i]);};
		}		
    ...
    onRender : function(ct, position){
        ...
        // ------ start add for rows per paging ---------
        // now add rows per paging information
