...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	...
	*/
	...
	,processSuccess:function(options, response, o) {
		...
		//Llenar variable con el mensaje obtenido
		if (response && response.responseText) {
			var resp = Ext.decode(response.responseText);
			if(resp.actionMessages){
				this.actionMessage = resp.actionMessages[0];
			}
		}
	...
	,processFailure:function(options, response, error) {
		...
		//Llenar variable con el mensaje de error obtenido
		if (response && response.responseText) {
			
			var resp = Ext.decode(response.responseText);
			if(resp.actionErrors){
				this.actionError = resp.actionErrors[0];
			}
	...
	,uploadCallback:function(options, success, response) {
		...
		//this.clientCallback(options, success, response);
