...
var comboTipoArchivo = new Ext.form.ComboBox({
	    ...
	    tooltip:getToolTipFromMap('tipoArchivo',helpMap,'Tipo de Archivo'),
	...
	var formPanel = new Ext.FormPanel({
		...
		var ventana = new Ext.Window ({
		    id:'winArchiDigId',
		    title: getLabelFromMap('winArchiDigId', helpMap,'Anexar Archivo Digitalizado'),
			//title: '<span style="color:black;font-size:12px;">Anexar Archivo Digitalizado</span>',
		    width: 500,
		    modal: true,
		    autoHeight: true,
		    items: [formPanel],
		      buttonAlign:'center',
		       buttons: [ {
		        text:getLabelFromMap('BtnCargar',helpMap,'Cargar'),
		        tooltip: getToolTipFromMap('BtnCargar',helpMap,'Cargar'),
				handler: function(){
                if (formPanel.form.isValid()) {
                	...
                	if (_files.getCount()) { //Si hay archivos para subir
                		//alert("hay archivos para subir al directorio: " + codigoDirectorioSubida);
	                	var _url = "cmd=upload&" + 
	                				"directorio=" + codigoDirectorioSubida; //+ "&" +
	                				//"nombre=" + Ext.getCmp('dsArchivo').getValue();

	                	//alert("action + url:  " + _ACTION_AGREGAR_ARCHIVO + "?" + _url);
