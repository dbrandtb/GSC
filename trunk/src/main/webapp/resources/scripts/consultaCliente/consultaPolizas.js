function obtenerPolizas(_cdperson){	
			...
			buttons:[
		        		{
		            		...
		            		handler:function(){
		            			if(getSelectedRecord(grid) != null){
		            				if(CONSULTA_CLIENTE_SCREEN){
		            					//Estoy en la pantalla Consulta de Cliente.
		            					//Codificar PKG_ENCUESTA.P_VALIDA_TCONFIGENCUESTA para ir a la pantalla CatBo_IngresarEncuesta.
		            					valida_tconfigencuesta(getSelectedRecord(grid),getSelectedRecord(grid).get('cdunieco'),getSelectedRecord(grid).get('cdramo'),_cdperson);
		            					/*window.location= IR_INGRESAR_ENCUESTAS + '?cdunieco='+ storePolizas.data.items[0].json.cdunieco +
																				 '&cdramo='+storePolizas.data.items[0].json.cdramo +
																				 '&nmpoliza='+storePolizas.data.items[0].json.nmpoliza+
																				 '&estado='+storePolizas.data.items[0].json.estado+
																				 '&cdperson='+_cdperson; */
		            					ventana.close();
		            				}else{
		            					//Estoy en la pantalla Alta de caso
			                			Ext.getCmp('acTFPolizaId').setValue(getSelectedRecord(grid).get('nmpoliex'));
			                			Ext.getCmp('acTFIncisoId').setValue(getSelectedRecord(grid).get('nmsituaext'));
			                			Ext.getCmp('cduniecoHiddenId').setValue(getSelectedRecord(grid).get('cdunieco'));
			                			Ext.getCmp('cdramoHiddenId').setValue(getSelectedRecord(grid).get('cdramo'));
			                						                			
			                			Ext.getCmp('estadoHiddenId').setValue(getSelectedRecord(grid).get('estado'));
			                			Ext.getCmp('nmpoliexHiddenId').setValue(getSelectedRecord(grid).get('nmpoliex'));
			                			Ext.getCmp('nmsituaextHiddenId').setValue(getSelectedRecord(grid).get('nmsituaext'));
			                			Ext.getCmp('nmsbsitextHiddenId').setValue(getSelectedRecord(grid).get('nmsbsitext'));
			                			Ext.getCmp('nmpolizaHiddenId').setValue(getSelectedRecord(grid).get('nmpoliza'));

			                			Ext.getCmp('acCmbTareaId').store.load({
		                												params:{cdUniEco: getSelectedRecord(grid).get('cdunieco'),
		                														cdRamo: getSelectedRecord(grid).get('cdramo')
		                														}
																		});
										
										Ext.getCmp('acChkbxPolizaId').setValue(false);
										ventana.close();
		            				}		            				
		                		}
		                		else{
		            	...
		            	},
		            	{
						    ...
						    handler:function(){
						    	if(getSelectedRecord(grid) != null){
   									//window.location = _IR_BUSQUEDA_POLIZA +'?_cdperson='+_cdperson +'&nmpoliex='+getSelectedRecord(grid).get('nmpoliex');
   									window.location = _IR_DETALLE_POLIZA +
										'?cdUnieco='+	getSelectedRecord(grid).get('cdunieco')+
										'&cdRamo='+		getSelectedRecord(grid).get('cdramo')+
										'&estado='+		getSelectedRecord(grid).get('estado')+
										'&nmPoliza='+	getSelectedRecord(grid).get('nmpoliza')+
										'&poliza='+		getSelectedRecord(grid).get('nmpoliex')+
										'&producto='+	getSelectedRecord(grid).get('dsramo')+
   										'&aseguradora='+getSelectedRecord(grid).get('dsunieco');
						    	}else{
	                				Ext.MessageBox.alert(getLabelFromMap('400000', helpMap,'Aviso'),getLabelFromMap('400011', helpMap,'Debe seleccionar un registro para realizar esta operaci&oacute;n'));
						    	}
   							}
	...
	var formPanel = new Ext.FormPanel({
		...
		/*grid.store.load({
			...
			callback:function(){
				var _dsunieco = (storePolizas.reader.jsonData.MListPolizas!=null)?storePolizas.reader.jsonData.MListPolizas[0].dsunieco:"";
				var _dsramo = (storePolizas.reader.jsonData.MListPolizas!=null)?storePolizas.reader.jsonData.MListPolizas[0].dsramo:""; 
	...
	var registro;	
	var _codpersonas;
	function valida_tconfigencuesta(record,_cdunieco, _cdramo,_cdperson){
		var _params = "cdModulo="+"";
		_params += "&cdUnieco="+_cdunieco;
		_params += "&cdRamo="+_cdramo;
		_params += "&cdElemento="+CDELEMENTO;
		registro = record;
		_codpersonas=_cdperson;
  ...
  function cbkValidar (_success, _message, _response) {
  	...
  	//alert(registro.get('nmpoliex'));
  		if (_success) {
			if(Ext.util.JSON.decode(_response).validaMio==1){
				window.location = IR_INGRESAR_ENCUESTAS + '?cdunieco='+registro.get('cdunieco')+//+ storePolizas.data.items[0].json.cdunieco +
														 '&cdramo='+registro.get('cdramo')+//+storePolizas.data.items[0].json.cdramo +
														 '&nmpoliza='+registro.get('nmpoliza')+//+storePolizas.data.items[0].json.nmpoliza+
														 '&estado='+registro.get('estado')+//+storePolizas.data.items[0].json.estado+
														 '&nmpoliex='+registro.get('nmpoliex')+//storePolizas.data.items[0].json.nmpoliex+
										 				 '&dsunieco='+registro.get('dsunieco')+//+storePolizas.data.items[0].json.dsunieco+
										 				 '&dsramo='+registro.get('dsramo')+//+storePolizas.data.items[0].json.dsramo+
										 				 '&nombrePersona='+registro.get('nombrePersona')+//+storePolizas.data.items[0].json.nombrePersona+
														 '&cdperson='+_codpersonas;//'+registro.get('cdperson');
       			/*window.location = IR_INGRESAR_ENCUESTAS + '?cdunieco='+ storePolizas.data.items[0].json.cdunieco +
														 '&cdramo='+storePolizas.data.items[0].json.cdramo +
														 '&nmpoliza='+storePolizas.data.items[0].json.nmpoliza+
														 '&estado='+storePolizas.data.items[0].json.estado+
														 '&nmpoliex='+storePolizas.data.items[0].json.nmpoliex+
										 				 '&dsunieco='+storePolizas.data.items[0].json.dsunieco+
										 				 '&dsramo='+storePolizas.data.items[0].json.dsramo+
										 				 '&nombrePersona='+storePolizas.data.items[0].json.nombrePersona+
														 '&cdperson='+CDELEMENTO;*/
		   ...
		   //alert(Ext.util.JSON.decode(_response).valida_i);
	...
	var cdPersonParam;
	function reloadGridPoliza(){
		var _params = {cdPerson:(cdPersonParam)?cdPersonParam:CODIGO_PERSONA,
					   cdElemento:""};		
