...
ExpresionesVentanaValidaciones = function(storeValidaciones,rec,row) {
var codigoExpresion;
if(row!=null){
	//alert("row"+row);
	...
	storeValidaciones.on('load', function(){
                           //var record = storeValidaciones.getAt(row);
                           Ext.getCmp('cabecera-validaciones-form').getForm().loadRecord(record);
		...
		*************************/
    	...
    	storeExpresion.on('load', function(){
                           ...
                           Ext.getCmp('top-validaciones').getForm().loadRecord(rec);
                           if(swRecalcular == 'S'){
	...
	***************************************************************/
	var nombreValidaciones = new Ext.form.TextField({
       ...
       var tipoValidaciones = new Ext.form.ComboBox({
   		...
   		tipods.load();
   	/*
   	tipoValidaciones.on('select',function(record){
	var valor=record.get('claveTipo');
	alert('valor ='+valor);
	Ext.getCmp('hidden-combo-tipo-validaciones').setValue(valor);
	});
	...
	var cabecera = new Ext.form.FormPanel({
                ...
                items: [{xtype:'hidden',id:'hidden-descripcion-expresion',name:'descripcionExpresion'},
						...
						{xtype:'hidden',id:'hidden-combo-tipo-validaciones',name:'tipo'},
						{
 				    		border: false,
 				    		layout: 'form',
				    		items: [{
