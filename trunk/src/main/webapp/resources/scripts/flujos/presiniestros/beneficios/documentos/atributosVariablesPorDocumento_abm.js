...
if(record){
	if(!Ext.isEmpty(record.get('cdAtribu'))){
		if(record.get('cdAtribu') == '1' || record.get('cdAtribu') == '2'){
			Ext.MessageBox.alert('Aviso','No puede editar este atributo, ya que es un valor por default');
			return;
		}
	}
	tituloForm = '<span style="color:black;font-size:12px;">'+getLabelFromMap('',helpMap,'Editar Atributo Variable por Tipo de Documento')+'</span>'
}else{
	tituloForm = '<span style="color:black;font-size:12px;">'+getLabelFromMap('',helpMap,'Agregar Atributo Variable por Tipo de Documento')+'</span>'
}
