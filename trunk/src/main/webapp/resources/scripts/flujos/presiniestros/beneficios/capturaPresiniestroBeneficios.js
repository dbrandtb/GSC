...
var gridDocs;
	...
	var textFolioAA = new Ext.form.TextField({
        fieldLabel: getLabelFromMap('',helpMap,'FolioAA'),
        tooltip:getToolTipFromMap('',helpMap,'FolioAA del Pre Siniestro'), 
        width: 100,
        value: BENEFICIO.folioAA,
        disabled: true
	});
	...
	var subinciso = new Ext.form.TextField({
        fieldLabel: getLabelFromMap('',helpMap,'Subinciso'),
        tooltip:getToolTipFromMap('',helpMap,'Subinciso del Pre Siniestro'), 
        width: 100,
        value: BENEFICIO.subinciso,
        disabled: true
	});
    ...
    var storePadecimiento = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
			url: _ACTION_COMBO_PADECIMIENTOS
        }),
        reader: new Ext.data.JsonReader({
            root: 'listaPadecimientos',
            id: 'value'
	        },[
           {name: 'otclave', type: 'string',mapping:'value'},
           {name: 'otvalor', type: 'string',mapping:'label'}
    ...
    var comboPadecimiento =new Ext.form.ComboBox({
    	                    //tpl: '<tpl for="."><div ext:qtip="{value}. {nick}" class="x-combo-list-item">{value}</div></tpl>',
							store: storePadecimiento,
						    displayField:'otvalor',
						    valueField: 'otvalor',
						    allowBlank:false,
					    	typeAhead: true,
					    	mode: 'local',
					    	triggerAction: 'all',
						    emptyText:'Seleccione un Padecimiento...',
					    	selectOnFocus:true,
					    	forceSelection:true,
					    	width:250,
						    fieldLabel: 'Padecimiento',
						    disabled: _UPDATE_MODE?true:false,
					    	hiddenName:'beneficio.dsPadecimiento'
	});
	...
	var descripcion = new Ext.form.TextArea({
        fieldLabel: getLabelFromMap('',helpMap,'Descripci&oacute;n del Tr&aacute;mite'),
        tooltip:getToolTipFromMap('',helpMap,'Descripci&oacute;n del Tr&aacute;mite del Pre Siniestro'), 
        name: 'beneficio.descripcionTramite',
        allowBlank:false,
        width: 310,
        height: 80,
        maxLength: 500,
	...
	if(_UPDATE_MODE){
		...
		fechaPrimerGasto.setValue(BENEFICIO.fechaPrimerGasto);
		comboPadecimiento.setValue(BENEFICIO.dsPadecimiento);
		...
		connect.request ({
			...
			callback: function (options, success, response) {				   
				var descripcionTramite = Ext.util.JSON.decode(response.responseText).mensajeRespuesta;
				var obs = Ext.util.JSON.decode(response.responseText).mensajeAux; 
				descripcion.setValue(descripcionTramite);
				observaciones.setValue(obs);
			}
		});
		
	}else {
		storePadecimiento.load();
	}
			...
			buttons:[
            		...
            		handler:function()
            				...
            				if(gridDocs.getSelectionModel().hasSelection()){
            					...
            					Ext.MessageBox.confirm('Eliminar', 'Se elimina&aacute; el documento seleccionado de la lista', function(btn) {
				                   if (btn == "yes") {
				               			...
				               			conn.request({
										    ...
										    callback: function(options, success, response) {
												if (Ext.util.JSON.decode(response.responseText).success != false) {
													//Ext.Msg.alert("Aviso", /*Ext.util.JSON.decode(response.responseText).mensajeRespuesta*/'Documento eliminado de la lista', function (){ storeGrid.load();	});
	...
	var formPresiniestros = new Ext.form.FormPanel({
	      ...
	      url: _ACTION_GUARDA_PREBEN,
	      title: '<span style="color:black;font-size:12px;">'+getLabelFromMap('',helpMap,'Pre Siniestros Beneficios')+'</span>',
      		    		...
      		    		items:[
	      		    		{
						    	columnWidth:.33,
		              			layout: 'form', 
		      		        	items:[noSiniestroAseg, fecha, poliza]
							},
							{
								columnWidth:.33,
		              			layout: 'form',
		              			items:[textFolio, aseguradora, inciso]
	              			},
							{
								columnWidth:.33,
		              			layout: 'form',
		              			items:[textFolioAA, ramo, subinciso]
	              			}
	              		 ]
              			},{
      		        	layout:'column',
      		    		...
      		    		items:[
      		    			{
						    	...
						    	columnWidth:.60,
		              			layout: 'form', 
		      		        	items:[comboTipoTramite, comboPadecimiento, titular, corpo, 
		      		        			asegurado, reportante, descripcion, observaciones]
							},
							{
								columnWidth:.36,
		              			layout: 'form',
		              			items:[fechaPrimerGasto,
		              				   {layout:'column', html:'<br><br><br><br><br><br><br>'},		              			
		              				   telefono]
	              			}
	              		 ]
              			},{
						 ...
						 handler: function(){
							...
							if(_UPDATE_MODE){
								parametros = { 
								 				...
								 				'beneficio.folioAA' : BENEFICIO.folioAA,
								 				...
								 				'beneficio.subinciso' : BENEFICIO.subinciso,
								 				...
								 				'beneficio.fechaPrimerGasto' : BENEFICIO.fechaPrimerGasto,
								 				'beneficio.dsPadecimiento' : BENEFICIO.dsPadecimiento,
