...
var gridBusquedaAtr;
	...
	var formBusquedaAtr = new Ext.form.FormPanel({
	      ...
	      title: '<span style="color:black;font-size:12px;">'+getLabelFromMap('',helpMap,'Atributos Variables por Tipo de Documento')+'</span>',
			        ...
			        buttons:[
			        	{
					     ...
					     tooltip: getToolTipFromMap('',helpMap,'Busca los atributos de este Tipo de Documento seg&uacute;n los crit&eacute;rios de b&uacute;squeda'),
  						 handler: function(){
							...
							storeGridAtr.load({
									...
									callback: function(r, options, success){				
										if(!success){
											var resp = Ext.util.JSON.decode(action.response.responseText).mensajeRespuesta
											Ext.MessageBox.alert('Aviso',Ext.isEmpty(resp)?'No se encontraron registros':resp);
	...
	gridBusquedaAtr = new Ext.grid.GridPanel({
			...
			buttons:[
            		...
            		handler:function()
            				{
            				if(gridBusquedaAtr.getSelectionModel().hasSelection()){
            					var cdAtributoEliminar = gridBusquedaAtr.getSelectionModel().getSelected().get('cdAtribu');
            					
            					if(!Ext.isEmpty(cdAtributoEliminar)){
									if(cdAtributoEliminar == '1' || cdAtributoEliminar == '2'){
										Ext.MessageBox.alert('Aviso','No puede eliminar este atributo, ya que es un valor por default');
										return;
									}
								}
					...
					tooltip: getToolTipFromMap('',helpMap,'Exporta los Atributos Variables del Tipo de Documento actual'),
                    handler:function(){}
	            	}],
	    	width:670,
	    	frame:true,
