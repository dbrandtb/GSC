package mx.com.aon.catbo.test.model;

public class ReciboVO{
	private String count;
	
	private String nmpoliza;
	private String nmsuplem;
	private String nmrecibo;
	private String feinicio;
	
	// GETTERS AND SETTERS
	public String getCount() {return count;}
	public void setCount(String count) {this.count = count;}

	public String getNmpoliza() {return nmpoliza;}
	public void setNmpoliza(String nmpoliza) {this.nmpoliza = nmpoliza;}
	public String getNmsuplem() {return nmsuplem;}
	public void setNmsuplem(String nmsuplem) {this.nmsuplem = nmsuplem;}
	public String getNmrecibo() {return nmrecibo;}
	public void setNmrecibo(String nmrecibo) {this.nmrecibo = nmrecibo;}
	public String getFeinicio() {return feinicio;}
	public void setFeinicio(String feinicio) {this.feinicio = feinicio;}
}
