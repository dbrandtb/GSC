...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="rows">
		<xsl:for-each select="row">
			<item-list
				xsi:type="java:mx.com.aon.portal.model.ConfiguracionNumeracionEndososVO"
				xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                ...
                <xsl:if test="@NMINICIAL">
                    <xsl:element name="nm-inicial">
                        <xsl:value-of select="@NMINICIAL" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@NMFINAL">
                    <xsl:element name="nm-final">
                        <xsl:value-of select="@NMFINAL" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@NMACTUAL">
                    <xsl:element name="nm-actual">
                        <xsl:value-of select="@NMACTUAL" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@OTEXPRES">
                    <xsl:element name="ot-expres">
