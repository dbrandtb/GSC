...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
            <cursor xsi:type="java:mx.com.royalsun.newalea.xml.cacheProducto.jaxb.impl.TipoRevalorizacionTypeImpl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">

                <xsl:if test="@CDTIPREV">
                    <xsl:element name="cdtiprev">
                        <xsl:value-of select="@CDTIPREV" />
                    </xsl:element>
                </xsl:if>
                <xsl:if test="@NMMINIMO">
                    <xsl:element name="nmminimo">
                        <xsl:value-of select="@NMMINIMO" />
                    </xsl:element>

                </xsl:if>
                <xsl:if test="@NMMAXIMO">
                    <xsl:element name="nmmaximo">
