    ...
    <xsl:template match="rows">
        <xsl:for-each select="row">
            <item-list xsi:type="java:mx.com.aon.catbo.model.CasoVO" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
                ...
                <xsl:if test="@INDSEMAFOROCOLOR">
                    <xsl:element name="ind-semaf-color">
                        <xsl:value-of select="@INDSEMAFOROCOLOR" />
                    </xsl:element>
                </xsl:if>
                ...
                <xsl:if test="@DSOBSERVACION">
                    <xsl:element name="observacion">
                        <xsl:value-of select="@DSOBSERVACION" />
                    </xsl:element>
                </xsl:if>
